...
class VKAccountBackupService:
    ...
    async def create_accounts(self, user_id: int, vk_creds: str):
        ...
        for account_log_pass in added_accounts_log_pass:
            ...
            task = chain(
                get_vk_account_curl.s(vk_account.id, account_log_pass.login,
                                      AuthService().decrypt_data(current_cred.encrypted_password)),
    ...
    async def get_random_account_backup_access_token(self):
        all_vk_accounts_backup = await self.database.vk_account.get_all_filtered(account_type="backup")
