...
def get_group_id(access_token: str, vk_account_id: int, proxy: str = None):
    ...
    groups = vk.groups.get(user_id=vk_account_id, extended=1, fields=["is_admin"])
    admin_groups = [group for group in groups["items"] if group.get("is_admin") == 1]
    group_id = admin_groups[0]["id"]
    print(f'VK ACCOUNT #{vk_account_id} admin_groups: {admin_groups}')
    ...
    try:
        ...
        with SyncSessionLocal() as session:
            livelogadd_sync(
                ...
                user_id,
                "account",
                "Профиль VK аккаунта обновлён",
                (
                    f"account_id={vk_account_id_database}; type={account_type_end}; "
        ...
        with SyncSessionLocal() as session:
            livelogadd_sync(
                ...
                user_id,
                "account",
                "Ошибка разбора профиля VK аккаунта",
                f"account_id={vk_account_id_database}; type={account_type_end}; error={e}",
            )
        raise
